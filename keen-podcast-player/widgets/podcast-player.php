...
class Podcast_Player extends Widget_Base {
    ...
    public function get_title() {
        return esc_html__( 'Podcast Player', 'keen-podcast-player' );
    }
    ...
    protected function register_controls() {
        $this->start_controls_section( 'section_content', [
            'label' => esc_html__( 'Content', 'keen-podcast-player' ),
            'tab'   => Controls_Manager::TAB_CONTENT,
        ] );

        $this->add_control( 'audio_file', [
            'label'      => esc_html__( 'Audio File', 'keen-podcast-player' ),
            'type'       => Controls_Manager::MEDIA,
            'media_type' => 'audio',
        ] );

        $this->add_control( 'episode_title', [
            'label'       => esc_html__( 'Episode Title', 'keen-podcast-player' ),
            'type'        => Controls_Manager::TEXT,
            'placeholder' => esc_html__( 'Defaults to file name', 'keen-podcast-player' ),
        ] );

        $this->end_controls_section();

        $this->start_controls_section( 'section_style', [
            'label' => esc_html__( 'Style', 'keen-podcast-player' ),
            'tab'   => Controls_Manager::TAB_STYLE,
        ] );

        $this->add_control( 'accent_color', [
            'label'     => esc_html__( 'Accent Color', 'keen-podcast-player' ),
        ...
        $this->add_control( 'hover_color', [
            'label'     => esc_html__( 'Button Hover Color', 'keen-podcast-player' ),
            'type'      => Controls_Manager::COLOR,
            'default'   => '#ffe641',
            'selectors' => [
        ...
        $this->add_control( 'background_color', [
            'label'     => esc_html__( 'Background', 'keen-podcast-player' ),
            'type'      => Controls_Manager::COLOR,
            'default'   => '#ffffff',
            'selectors' => [
        ...
        $this->add_control( 'border_radius', [
            'label'      => esc_html__( 'Border Radius', 'keen-podcast-player' ),
    ...
    protected function render() {
        ...
        if ( empty( $audio_url ) ) {
            if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
                echo '<div class="spp-editor-notice">'
                    . esc_html__( 'Podcast Player: select an audio file in the Content tab.', 'keen-podcast-player' )
                    . '</div>';
            }
            return;
        }
        ...
        <div class="spp-player">
            ...
            <div class="spp-bar">
                ...
                <button class="spp-play-btn" aria-label="<?php esc_attr_e( 'Play', 'keen-podcast-player' ); ?>">
            ...
            <div class="spp-progress-row">
                <div class="spp-track"
                     role="progressbar"
                     aria-label="<?php esc_attr_e( 'Playback progress', 'keen-podcast-player' ); ?>"
                     aria-valuemin="0"
                     aria-valuemax="100"
                     aria-valuenow="0"
                     tabindex="0">
                ...
                <button class="spp-speed" aria-label="<?php esc_attr_e( 'Playback speed', 'keen-podcast-player' ); ?>">1×</button>
            </div>
        </div>
        <?php
    }
